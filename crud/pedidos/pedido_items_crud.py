...
def get_pedido_item(db: Session, item_id: UUID) -> Optional[PEDIDO_ITEMS]:
    return db.get(PEDIDO_ITEMS, item_id)


def list_pedido_items(
    db: Session, pedido_id: UUID, skip: int = 0, limit: int = 100
) -> List[PEDIDO_ITEMS]:
    return (
...
def update_pedido_item(
    db: Session, item_id: UUID, data: PedidoItemUpdate
