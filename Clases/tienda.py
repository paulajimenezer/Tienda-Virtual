...
class DescuentoPorcentaje(Descuento):
    ...
    def aplicar_descuento(self, total: float) -> float:
        return total * (1 - self.porcentaje / 100) #Este metodo aplica un descuento del 10% al total del monto
...
class DescuentoFijo(Descuento):
    ...
    def aplicar_descuento(self, total: float) -> float:
        return max(0, total - self.monto) #Este metodo aplica un descuento fijo de 20$ y con max 0 impide que el resultado sea negativo
...
class Tienda:
    def __init__(self):
        ...
        self.productos_disponibles = [
            ...
            ProductoElectronico(2, "Smartphone", 899.99, "Samsung", "Galaxy S24", 12, 15),
            ProductoElectronico(3, "Laptop Ultrabook", 1200.50, "Dell", "XPS 13", 24, 8),
            ProductoElectronico(4, "Auriculares Bluetooth", 249.99, "Sony", "WH-1000XM5", 12, 20),
            ProductoElectronico(5, "Tablet", 450.75, "Apple", "iPad Air", 12, 12),
    
            # Productos de Ropa
            ProductoRopa(6, "Camisa Formal", 45.99, "Zara", "Algodón", "M", "Azul", 10),
            ProductoRopa(7, "Jeans Clásicos", 75.50, "Levi's", "Denim", "32", "Negro", 25),
            ProductoRopa(8, "Chaqueta Deportiva", 120.00, "Nike", "Poliéster", "L", "Gris", 8),
            ProductoRopa(9, "Vestido Casual", 65.99, "H&M", "Algodón", "S", "Rojo", 15),
            ProductoRopa(10, "Zapatos Deportivos", 180.25, "Adidas", "Sintético", "42", "Blanco", 18),
    
            # Productos de Comida
            ProductoComida(11, "Manzana Royal", 2.09, "Fruta", 200, "2024-12-31", 50),
            ProductoComida(12, "Cereal Integral", 8.75, "Desayuno", 500, "2025-06-15", 30),
            ProductoComida(13, "Yogurt Natural", 3.50, "Lácteo", 250, "2024-09-15", 40),
            ProductoComida(14, "Pasta Italiana", 4.25, "Carbohidrato", 400, "2025-12-01", 35),
            ProductoComida(15, "Chocolate Premium", 12.99, "Dulce", 100, "2025-03-20", 22)
        ]
        self.carrito = Carrito()

    def mostrar_menu(self): #Menú principal, con todas las opciones disponibles para el usuario
        while True:
            print("\n--- MENÚ TIENDA ONLINE ---")
    ...
    def ver_productos(self): #Muestra todos los productos disponibles en la tienda recorriendolos en un for
        print("\n--- PRODUCTOS DISPONIBLES ---")
        for i, producto in enumerate(self.productos_disponibles, start=1):
            print(f"[{i}] {producto}")

    def agregar_al_carrito(self): #Pide al usuario el id del producto y la cantidad, valida que el id sea correcto y llama al metodo agregar_producto del carrito
        ...
        if entrada_producto.isdigit():
            ...
            if id_producto < 1 or id_producto > len(self.productos_disponibles):
                print(f"Producto no válido. Debe seleccionar un número entre 1 y {len(self.productos_disponibles)}.")
                return
        else:
        ...
        self.carrito.agregar_producto(self.productos_disponibles[indice_producto], cantidad)

    def eliminar_del_carrito(self): #Pide al usuario el nombre del producto a eliminar, busca el producto en la lista de productos disponibles y llama al metodo eliminar_producto del carrito
    ...
    def calcular_total(self): #Calcula el total del carrito recorriendo los productos y sumando su precio por la cantidad
        total = 0
        for producto, cantidad in self.carrito._items:
            total += producto.get_precio() * cantidad
        return total

    def gestionar_descuento(self): #Muestra un menu de descuentos disponibles, pide al usuario que seleccione uno y aplica el descuento al total
